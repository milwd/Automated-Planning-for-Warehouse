...
(define (domain gripper-continuous)
    ...
    (:action pickup_group
        :parameters (?b - ball ?g - group ?curr_g - group ?m - mover)
        :precondition (and 
            (belongtogroup ?b ?g) (hasgoup ?b)
            (not (moving ?m)) (topositive ?m) (= (at-robby ?m) (position ?b)) (not (isloaded ?b)) (> (position ?b) 0) (<= (weight ?b) 50) (not (isfragile ?b)) (free ?m)
            (exists (?b - ball) (and (belongtogroup ?b ?g) (not(isloaded ?b))))
        ...
        :effect (and 
            ...
            (when(and(not(iscurrentgroup ?g ?curr_g)) (not(exists (?b - ball) (and (belongtogroup ?b ?g) (not(isloaded ?b))))))
